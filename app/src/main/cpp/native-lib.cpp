...
Java_com_example_iguanas_Iguanas_initDetector(JNIEnv* env, jobject p_this, jobject assetManager) {

    char *buffer = nullptr;
    long size = 0;

    //Se lee la red neuronal para detectar los objetos que tiene extencion tflite
...
// Para eliminar el detector
extern "C" JNIEXPORT void JNICALL
Java_com_example_iguanas_Iguanas_destroyDetector(JNIEnv* env, jobject p_this, jlong p_native_ptr) {
    if (p_native_ptr)
        delete (ObjectDetector *) p_native_ptr;
}

//Se utiliza para cuando ya se detecta un objeto
extern "C" JNIEXPORT jfloatArray JNICALL
Java_com_example_iguanas_Iguanas_detect(JNIEnv* env, jobject p_this, jlong detectorAddr, jbyteArray src, int width, int height) {
